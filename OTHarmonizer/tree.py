...
def convert_to_zssNode(my_node):
    labels = my_node.label.split('&')
    labels_sorted = sorted(labels)
    new_label = '&'.join(labels_sorted)
    zss_node = zssNode(new_label)

    sorted_children = sorted(my_node.children, key=lambda x: '&'.join(sorted(x.label.split('&'))))
