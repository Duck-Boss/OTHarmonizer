    ...
    for c2 in C2:
        ...
        for c1 in C1:
            if D1.loc[c1, c2] > 0.05:
                found = True
                break
#         if (not found) and (c2 not in equal.values()) and (c2 not in flat_list) and (c2 not in Parent_children):
    ...
    for c1 in C1:
        ...
        for c2 in C2:
            if D2.loc[c2, c1] > 0.05:
                found = True
                break
#         if (not found) and (c1 not in equal) and (c1 not in flat_list) and (c1 not in Parent_children):
...
def do_harmonization(adata, annotation_index, batch_index, sample_size=100, transmission_ratio=0.4, batch_order=None):
    ...
    # Combine batch information with annotations
    ...
    # Extract all batches and sort them by annotation count in descending order
    ...
    # Sort batches based on batch_order or annotation count
    if batch_order is None:
        # Sort by annotation count in descending order
    ...
    for i in range(len(batch_names)):
        if i == 0:
            ...
            # Process the second batch and initialize the tree
            ...
            # Calculate partial optimal transport and analyze transmission
